...
~~Güvenlik - JWT Auth: Basic Auth kaldırılarak JWT/OAuth2 implementasyonu.~~ ✅ **TAMAMLANDI (2026-07-21)** — `Services/auth_service.py` eklendi: bcrypt parola doğrulama + PyJWT (HS256) token. Panel HTTP Basic Auth'tan httpOnly çerezli JWT oturumuna geçti; login/logout ekranları, oturum süresi (JWT_EXPIRE_HOURS). `require_dashboard_auth` imzası korunduğu için 22 rota değişmedi (DRY). Parola `DASHBOARD_PASSWORD_HASH` (bcrypt) olarak saklanır; `generate_password_hash.py` ile üretilir. Doğrulama: `debug_auth.py` (16/16, alg=none ve kurcalama dahil) + uçtan uca akış testi (8/8). Not: tek kullanıcı; çoklu kullanıcı/RBAC multi-tenancy ile gelecek.
